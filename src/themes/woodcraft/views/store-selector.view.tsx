    ...
    if (locations) {
        ...
        return (
            <RichTextComponent
                ...
                text={isSeeAllStore ? resources.storeAllCountMessage
                    .replace('{count}', `<span class='ms-store-select__found-locations__count'>${locations.length.toString()}</span>`) : storeCountMessage
                    .replace('{count}', `<span class='ms-store-select__found-locations__count'>${locations.length.toString()}</span>`)
                    .replace('{radius}', `<span class='ms-store-select__found-locations__radius'>${searchRadius.toString()}</span>`)}
            />
        );
    }
...
const buildStoreHoursDay = (displayTime: IHoursDisplayInfo, storeHoursDayPeriodString: string, storeHoursDayString: string) => {
    return (
        <RichTextComponent
            ...
            text={displayTime.endDayShort ? storeHoursDayPeriodString
                .replace('{startDay}', `<abbr title=${displayTime.startDayFull}>${displayTime.startDayShort}</abbr>`)
                .replace('{endDay}',
                    `<abbr title=${displayTime.endDayFull ? displayTime.endDayFull : ''}>${displayTime.endDayShort}</abbr>`) : storeHoursDayString
                .replace('{startDay}', `<abbr title=${displayTime.startDayFull}>${displayTime.startDayShort}</abbr>`)}
        />
    );
};
            ...
            <div className='ms-store-select__store-hours-details-group'>
                {storeHoursList?.map(displayTime => (
                    <div className='ms-store-select__store-hours-details' key={JSON.stringify(displayTime)}>
                        ...
                        <div className='ms-store-select__store-hours-time'>
...
const PickUpMethodDropdownComponent = (props: IPickupComponent & IPickUpMethodResources) => {
    ...
    const renderLocationLinePickupButton = (filteredDeliveryMode: DeliveryOption | undefined) => {
        return (
            filteredDeliveryMode !== undefined ? <div className='ms-store-select__location-line-pickup-list-container'>
                <button
                    role='menuitem'
    ...
    return (
        <div
            ...
            data-selected={(pickUpStatesProps.state.filteredPickupMode !== undefined && filteredDeliveryMode !== undefined) || selectedStorePickUpOption !== ''}
            data-error={isError}>
            <div className='ms-store-select__location-line-pickup-label'>
                {pickUpMethodLabel}
            </div>
            {pickUpStatesProps.state.filteredPickupMode === undefined ? <div className='ms-store-select__location-line-pickup-list-container'>
                <button
                ...
                <ul
                    className={classnames('ms-store-select__location-line-pickup-list', isDropdownOpen ? 'show' : '')}>
                ...
                <div className='ms-store-select__location-line-pickup-list-error'>
                    {pickupOptionDropdownError}
                </div>
            </div> : renderLocationLinePickupButton(filteredDeliveryMode)}
            <button
                className='ms-store-select__location-line-pickup-menu'
                aria-label={pickupButtonAriaLabel}
    ...
    return (
        <div className='ms-store-select__location-line-item'>
            <div className='ms-store-select__location-line-item-header'>
                {storeLocatorView && <span className='ms-store-select__location-line-item-store-index'>
                    {index}
                </span>}
            ...
            <div className='ms-store-select__location-line-item-content'>
                <div className='ms-store-select__location-line-item-contact-info'>
                    <p className='ms-store-select__location-line-header'>
                    ...
                    {storeAddress && (
                        <div className='ms-store-select__location-line-shop-address-section' aria-label={storeAddress} role='button'>
                            <span className='msi msi-map-marker-alt ms-store-select__location-line-shop-address-glyph-icon' aria-hidden='true' />
                    ...
                    {phoneNumber && (
                        <div className='ms-store-select__location-line-shop-phone-section'>
                            ...
                            <span className='ms-store-select__location-line-shop-phone-text'>
                                {phoneNumber}
                            </span>
                        </div>
                    )}
                ...
                </div>
                {
                    ...
                    <div className='ms-store-select__location-line-item-store-availability'>
                        ...
                        {stockText && <p className='ms-store-select__location-line-stock-status'>
                            {stockText}
                        </p>}
                    </div>
                }
                {
                    isPreferredStoreEnabled && (isPreferredStore ? <button
                        role='checkbox'
...
const renderStoreLocation = (location: IStoreSelectorLocationLineItemProps,
    index: number, props: IStoreSelectorViewProps & IStoreSelectorProps<IStoreSelectorResources>,
    selectedLocationReference: React.RefObject<HTMLDivElement>): JSX.Element | undefined => {
    const {
        // eslint-disable-next-line @typescript-eslint/naming-convention -- Dependency from store-selector.tsx file
    ...
    const canSelectPickUpMethod = !ArrayExtensions.hasElements(deliveryOptionList) ? (
        <button
            ...
            onClick={onPickupInStoreClickHandler()}
            {...attributes}
        >
            {pickupInStoreButtonText}
        </button>
    ...
    return (
        <div className={classnames('ms-store-select__location-wrapper', { selected: selectedStoreLocationId === orgUnitLocation.OrgUnitNumber &&
            !props.isMobileView })}>
            <div
            ...
            {!storeLocatorView && (isCurrentLocation ? (
                <p className='ms-store-select__location-line-current-location'>
 ...
 * @returns The JSX Element.
 */
const renderStoreLocations = (props: IStoreSelectorViewProps & IStoreSelectorProps<IStoreSelectorResources>,
...
export const renderSelectedStore = (selectedLocationProps: ISelectedStoreViewProps,
    storeSelectorViewProps: IStoreSelectorViewProps & IStoreSelectorProps<IStoreSelectorResources>): JSX.Element => {
    const {
        storeDetailsMain,
        storeHeader,
        storeName,
    ...
    ) : selectedLocation && (<PickUpMethodDropdownComponent
        {...selectedLocation}
            ...
            <Node {...storeDetailsAvailabilityContainer}>
                ...
                </div>
                {
                    ...
                    <div className='ms-store-select__location-line-item-store-availability'>
                        ...
                        {stockStatus && <p className='ms-store-select__location-line-stock-status'>
                            {stockStatus}
                        </p>}
                    </div>
                }
...
const StoreSelectorViewComponent: React.FC<IStoreSelectorViewProps & IStoreSelectorProps<IStoreSelectorResources>> = props => {
    ...
    if (!isDesktopView || !isDesktopPort) {
        return (
            <Module {...ModuleProps} >
                <Node {...HeaderContainerProps}>
                    {HeaderProps}
                </Node>
                <Node {...BodyContainerProps}>
                    <Node {...storeResultContainerProps!}>
                        {locationsMessage && <StoreSelectorFoundLocationsMessageComponent {...(locationsMessage as React.ReactElement).props} />}
                        {search}
    ...
    return (
        <Module {...ModuleProps} >
            ...
            <Node {...BodyContainerProps}>
                <Node className={`ms-store-select__store_container ${data.storeSelectorStateManager.result?.listMapViewState.displayList ? 'show' : ''}`}>
                    {locationsMessage && <StoreSelectorFoundLocationsMessageComponent {...(locationsMessage as React.ReactElement).props} /> }
                    {search}
                    {state.isSearchInProgress ? spinner : <>
                        {locationsList ? renderStoreLocations(props, selectedLocationReference) : noLocationsMessage}
                ...
                <Node className={`ms-store-select__map_container ${data.storeSelectorStateManager.result?.listMapViewState.displayMap ? '' : 'hide'}`} >
                    {maps}
                </Node>
            </Node>
            <Node {...FooterContainerProps}>
